-- PostgreSQL 数据库重置脚本
-- 使用方法: 在 api/database 目录下执行: psql -U postgres -d union_staff -f reset.sql
-- 或设置编码后执行: set PGCLIENTENCODING=UTF8 && psql -U postgres -d union_staff -f api/database/reset.sql
...
DROP TABLE IF EXISTS mutual_aid_difficulty_applications CASCADE;
DROP TABLE IF EXISTS mutual_aid_applications CASCADE;
DROP TABLE IF EXISTS difficulty_applications CASCADE;
DROP TABLE IF EXISTS membership_applications CASCADE;
DROP TABLE IF EXISTS fee_authorizations CASCADE;
...
SELECT '数据库重置完成' AS result,
       (SELECT COUNT(*) FROM users) AS users_count,
       (SELECT COUNT(*) FROM modules) AS modules_count,
       (SELECT COUNT(*) FROM disease_types) AS disease_types_count,
       (SELECT COUNT(*) FROM difficulty_applications) AS difficulty_count,
       (SELECT COUNT(*) FROM mutual_aid_difficulty_applications) AS mutual_aid_difficulty_count;