...
export const MutualAidDifficultyAudit = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        ...
        const response = await fetch(`/api/mutual-aid-difficulty/applications?${queryString}`, {
  ...
  const handleView = async (row: Application) => {
    ...
    const detailResponse = await api.mutualAidDifficulty.getApplication(row.id)
    if (detailResponse.success && detailResponse.data.disease_type_id) {
      const duplicateResponse = await api.mutualAidDifficulty.checkDuplicate(detailResponse.data.disease_type_id, detailResponse.data.user_id)
      if (duplicateResponse.success && duplicateResponse.data.isDuplicate) {
        ...
        if (existingApp && existingApp.id !== row.id) {
          ...
          setDuplicateWarning(`该用户已有${diseaseName}的爱心帮扶申请（${statusLabel}），同一病种仅能申请一次`)
        }
      }
    }
  ...
  const handleGrassRootPass = async () => {
    ...
    try {
      await api.mutualAidDifficulty.updateApplication(selectedApp.id, {
  ...
  const handleMarkAsAudited = async (row: Application) => {
    try {
      const response = await api.mutualAidDifficulty.markAsAudited(row.id, !row.marked_as_audited)
      if (response.success) {
  ...
  const handleBatchMarkAsAudited = async () => {
    ...
    try {
      const response = await api.mutualAidDifficulty.batchMarkAsAudited(selectedIds, true)
      if (response.success) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          <FileText size={28} className="text-red-500 mr-3" />
          <h2 className="text-xl font-semibold text-gray-800">爱心帮扶审批</h2>
        </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="font-medium text-gray-800">爱心帮扶申请</h3>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 border border-gray-200 rounded-lg">
                  <div className="flex items-center">
                    <span className="px-2 py-0.5 text-xs rounded-full bg-red-100 text-red-700 mr-2">
            ...
            {selectedApp.family_income !== null || selectedApp.family_members ? (
              <div>
                <div className="flex items-center mb-3">
                  <Users size={18} className="text-red-600 mr-2" />
                  ...
                  <div className="space-y-4">
                    ...
                    {selectedApp.grass_root_signature && (
                      <div className="p-3 bg-red-50 rounded-lg border border-red-200">
                        ...
                        <p className="font-medium text-red-700">{selectedApp.grass_root_signature}</p>
                      </div>
                    )}
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">审核意见 *</label>
